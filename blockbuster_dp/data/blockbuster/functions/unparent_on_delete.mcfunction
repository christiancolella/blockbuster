...
scoreboard players operation @e[type=armor_stand,tag=block,scores={temp=3}] initial_offset_x += @e[type=armor_stand,tag=block,tag=has_parent,scores={global=2}] initial_offset_x
scoreboard players operation @e[type=armor_stand,tag=block,scores={temp=3}] initial_offset_x -= @e[type=armor_stand,tag=block,tag=!has_parent,scores={global=2}] initial_offset_x
execute if entity @e[type=armor_stand,tag=block,tag=!has_parent,scores={global=2},limit=1] run scoreboard players operation @e[type=armor_stand,tag=block,scores={temp=3}] initial_offset_x *= #-1 constants
scoreboard players operation @e[type=armor_stand,tag=block,scores={temp=3}] initial_offset_y += @e[type=armor_stand,tag=block,scores={global=2}] initial_offset_y

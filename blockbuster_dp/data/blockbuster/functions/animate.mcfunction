...
# increase time
scoreboard players add @e[type=armor_stand,tag=block,scores={play=1}] time 1
...
execute as @e[type=armor_stand,tag=block] if score @s from_pos_time = @s to_pos_time run scoreboard players add @s to_pos_time 1
